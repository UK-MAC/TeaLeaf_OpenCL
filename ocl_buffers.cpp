...
(void)
{
    size_t total_cells = (tile_x_cells+2*run_params.halo_allocate_depth+1) * (tile_y_cells+2*run_params.halo_allocate_depth+1);
    ...
    #define BUF1DX_ALLOC(name, x_e)     \
        BUF_ALLOC(name, (tile_x_cells+2*run_params.halo_allocate_depth+x_e) * sizeof(double))

    #define BUF1DY_ALLOC(name, y_e)     \
        BUF_ALLOC(name, (tile_y_cells+2*run_params.halo_allocate_depth+y_e) * sizeof(double))

    #define BUF2D_ALLOC(name, x_e, y_e) \
        BUF_ALLOC(name, (tile_x_cells+2*run_params.halo_allocate_depth+x_e) * (tile_y_cells+2*run_params.halo_allocate_depth+y_e) * sizeof(double))
    ...
    size_t lr_mpi_buf_sz = sizeof(double)*(tile_y_cells + 2*run_params.halo_allocate_depth)*run_params.halo_allocate_depth;
    size_t bt_mpi_buf_sz = sizeof(double)*(tile_x_cells + 2*run_params.halo_allocate_depth)*run_params.halo_allocate_depth;
