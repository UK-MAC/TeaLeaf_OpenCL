 ...
 const int coef)
{
    ...
    {
        u0[THARR2D(0, 0, 1)] = energy1[THARR2D(0, 0, 0)] * density1[THARR2D(0, 0, 0)];
        ...
        && column >= (x_min + 1))
        {
            ...
            Kx[THARR2D(0, 0, 1)] = (dens_left + dens_centre)/(2*dens_left*dens_centre);
            Ky[THARR2D(0, 0, 1)] = (dens_up + dens_centre)/(2*dens_up*dens_centre);
        }
    }
}
 ...
 __global       double * __restrict const un)
{
    ...
    && column >= (x_min + 1) - 1 && column <= (x_max + 1) + 1)
    {
        un[THARR2D(0, 0, 1)] = u1[THARR2D(0, 0, 0)];
    }
}
 ...
 __global       double * __restrict const error)
{
    ...
    {
        u1[THARR2D(0, 0, 0)] = (u0[THARR2D(0, 0, 1)]
            + Kx[THARR2D(1, 0, 1)]*rx*un[THARR2D( 1,  0, 1)]
            + Kx[THARR2D(0, 0, 1)]*rx*un[THARR2D(-1,  0, 1)]
            + Ky[THARR2D(0, 1, 1)]*rx*un[THARR2D( 0,  1, 1)]
            + Ky[THARR2D(0, 0, 1)]*rx*un[THARR2D( 0, -1, 1)])
