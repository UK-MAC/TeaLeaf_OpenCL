...
SUBROUTINE tea_allocate_buffers(chunk)
  ...
  bt_size = num_buffered*(chunks(chunk)%field%x_max + 2*max(2, halo_allocate_depth))*halo_allocate_depth
  lr_size = num_buffered*(chunks(chunk)%field%y_max + 2*max(2, halo_allocate_depth))*halo_allocate_depth
