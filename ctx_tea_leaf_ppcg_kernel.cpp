...
{
    chunks.at(fine_chunk)->ppcg_init(ch_alphas, ch_betas, theta, n_inner_steps);
}

void TeaCLContext::ppcg_init_sd_kernel
(void)
{
    chunks.at(fine_chunk)->ppcg_init_sd_kernel();
}

void TeaCLContext::tea_leaf_ppcg_inner_kernel
(int inner_step, int bounds_extra, const int* chunk_neighbours)
{
    chunks.at(fine_chunk)->tea_leaf_ppcg_inner_kernel(inner_step, bounds_extra,
        chunk_neighbours);
}
