...
!$    IF(OMP_GET_THREAD_NUM().EQ.0) THEN
  ...
  ! Note that the "correct" solution is with IEEE switched on, 1 task, 1 thread, Intel compiler on Ivy Bridge
          ...
          IF(test_problem.EQ.6) qa_diff=ABS((100.0_8*(temp/103.886391260053329688_8))-100.0_8) ! 500x500 20 steps
          IF(test_problem.EQ.7) qa_diff=ABS((100.0_8*(temp/90.5991903174078033_8))-100.0_8) ! 500x500 20 steps
